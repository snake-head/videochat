...
def loadvideo_decord(sample, sample_rate_scale=1, new_width=384, new_height=384, clip_len=8, frame_sample_rate=2,
                     num_segment=1):
    fname = sample
...
def loadvideo_decord_origin(sample, sample_rate_scale=1, new_width=384, new_height=384, clip_len=8, frame_sample_rate=2,
                            num_segment=1):
    fname = sample
...
def loadvideo_decord_time_segment(sample, segment_length=10, sample_rate_scale=1, new_width=384, new_height=384, frame_sample_rate=2):
    fname = sample
    ...
    for i in range(num_segment):
        ...
        index = np.arange(start_time * vr.get_avg_fps(), end_time * vr.get_avg_fps(), step=int(sample_rate_scale))
    ...
    if remaining_time > 0:  # 如果有剩余的时间，则将其作为一个单独的片段处理
        ...
        index = np.arange(start_time * vr.get_avg_fps(), len(vr), step=int(sample_rate_scale))
        all_index.append(list(index))
        vr.seek(0)
        buffer.append(vr.get_batch(list(index)).asnumpy())
